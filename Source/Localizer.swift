...
public class Localizer: LocalizerType, ReactiveCompatible {
    ...
    public func localized(_ string: String) -> Driver<String> {
        return currentLanguageCode.asDriver().map { _ in
    ...
    private init() {
        ...
        changeLanguage.distinctUntilChanged().asDriver(onErrorJustReturn: nil).drive(onNext: {
        ...
        currentLanguageCode.drive(onNext: { self.currentLanguageCodeValue = $0 }).disposed(by: disposeBag)
